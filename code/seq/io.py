...
class BedRecord:
    ...
    def to_bedpe_aux(self):
        assert self.format == BED_FILE_TYPE.BEDPE
        # 确保zygosity不为None，优先使用self.aux中的值，若为None则使用默认的UNK
        zygosity = self.aux.get('zygosity') or constants.ZYGOSITY.UNK
        return "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s" % (
            self.intervalA.chr_name,
            self.intervalA.start,
            self.intervalA.end,
            self.intervalB.chr_name,
            self.intervalB.start,
            self.intervalB.end,
            self.sv_type,
            self.aux['score'],
            constants.ZYGOSITY_GT_BED[zygosity]
        )
...
if __name__ == '__main__':
  ...
  # def to_bedpe_aux(self):
  #       assert self.format == BED_FILE_TYPE.BEDPE
  #       zygosity = self.aux['zygosity']
  #
  #       # 将 0 映射到 UNK（根据错误上下文推断）
  #       if zygosity in (0, 1):  # 假设 0 或 1 对应未知状态
  #           zygosity = constants.ZYGOSITY.UNK
  #
  #       return "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s" % (
  #           self.intervalA.chr_name,
  #           self.intervalA.start,
  ...
  #           self.sv_type,
  #           self.aux['score'],
  #           constants.ZYGOSITY_GT_BED.get(zygosity, constants.ZYGOSITY_GT_BED[constants.ZYGOSITY.UNK])  # 兜底逻辑
#         )